...
import {
  Send, Paperclip, Mic, X, CheckCircle2,
...
export default function SendMessagePage() {
  ...
  useEffect(() => {
    // Auto-select leader if only one open queue (for regular) or any leader (for emergency)
    if (leaders.length === 1) {
  ...
  const handleSend = async () => {
    ...
    if (!content.trim() && !mediaFile && !audioBlob) {
      toast.error('Please write a message or attach media');
      return;
    }
    ...
    try {
      ...
      if (mediaFile) {
        ...
        fd.append('media_type', mediaType || 'document');
      }
      ...
      if (!res.ok) {
        if (data.error === 'limit_reached') {
          toast.error('You have reached your 3 emergency messages for today.');
        } else if (data.error === 'already_sent') {
          toast.error('You have already sent a message in this queue.');
        } else {
  ...
  if (sent) {
    return (
      <div className="min-h-screen bg-[#0A0A0F] flex flex-col items-center justify-center p-6">
        ...
        >
          Your message has been delivered to {selectedLeader?.display_name}
        </motion.p>
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.8 }}
          className="text-[#5A5A72] text-xs mt-4"
        >
          Returning to home...
        </motion.p>
      </div>
  ...
  return (
    <AppShell showNav={false}>
      <PageHeader
        title={isEmergency ? emergencyConfig!.label : 'Send Message'}
        showBack
        showHelp
      />
      ...
      <div className="px-4 pb-8 max-w-lg mx-auto space-y-4">
        ...
        {isEmergency && emergencyConfig && (
          ...
          >
            <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0', emergencyConfig.badge)}>
              <emergencyConfig.icon className="w-4 h-4" />
            </div>
            <div>
              <p className={cn('text-sm font-semibold', emergencyConfig.color)}>{emergencyConfig.label}</p>
              <p className="text-[#9A9AB0] text-xs">This message bypasses the queue and is sent immediately</p>
            </div>
          </motion.div>
        )}
        ...
        >
          ...
          >
                ...
                <div className="flex-1 text-left">
                  ...
                  {!isEmergency && (
                    <p className="text-[#5A5A72] text-xs">
                      {openQueues.find(q => q.leader_id === selectedLeader.id)
                        ? 'Queue open'
                        : 'Emergency only'}
                    </p>
                  )}
            ...
            <ChevronDown className={cn(
              'w-4 h-4 text-[#5A5A72] transition-transform flex-shrink-0',
              showLeaderPicker && 'rotate-180'
            )} />
          </button>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        canSelect
                          ? 'hover:bg-[#C9A84C]/5 active:bg-[#C9A84C]/10'
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[#5A5A72] text-xs">
                          {isEmergency
                            ? 'Emergency message'
                            : hasOpenQueue ? '✓ Queue open' : 'Queue closed'}
                        </p>
                      </div>
        ...
        >
          <p className="text-xs text-[#5A5A72] uppercase tracking-wider mb-2 font-medium">
            Attach Media <span className="normal-case text-[#3A3A52]">(optional)</span>
          </p>
          ...
          {(mediaFile || audioBlob) ? (
                  ...
                  {mediaPreview && mediaType === 'photo' ? (
                    ...
                    <>
                      <p className="text-[#F0EDE8] text-sm font-medium truncate">
                        {mediaType === 'voice' ? 'Voice Note' : mediaFile?.name}
                      </p>
                      <p className="text-[#5A5A72] text-xs capitalize">
                        {mediaType} • {mediaFile ? `${(mediaFile.size / 1024).toFixed(1)} KB` : ''}
                      </p>
                    </>
                  )}
                </div>
            ...
            <div className="bg-red-500/8 border border-red-500/30 rounded-2xl p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[#9A9AB0] text-xs">{formatRecordingTime(recordingTime)}</p>
              </div>
              <button
                onClick={stopRecording}
                className="w-10 h-10 rounded-xl bg-red-500 flex items-center justify-center text-white"
              >
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { icon: Image, label: 'Photo', accept: 'image/*', type: 'photo' },
                { icon: FileVideo, label: 'Video', accept: 'video/*', type: 'video' },
                { icon: FileAudio, label: 'Audio', accept: 'audio/*', type: 'audio' },
                { icon: File, label: 'Doc', accept: '*', type: 'document' },
                ...
                <button
                  ...
                  className="bg-[#1A1A26] border border-[#2A2A3E] rounded-xl py-3 flex flex-col items-center gap-1.5 hover:border-[#C9A84C]/30 hover:bg-[#C9A84C]/5 transition-all active:scale-95"
                >
          ...
          {/* Voice note button */}
          {!mediaFile && !audioBlob && !isRecording && (
            <button
              onClick={startRecording}
              className="mt-2 w-full bg-[#1A1A26] border border-[#2A2A3E] rounded-xl py-3 flex items-center justify-center gap-2 hover:border-[#C9A84C]/30 hover:bg-[#C9A84C]/5 transition-all active:scale-[0.98]"
            >
        ...
        >
          <button
            ...
            className={cn(
              'w-full rounded-2xl py-4 flex items-center justify-center gap-2.5 font-semibold text-sm transition-all duration-200 active:scale-[0.98]',
              isEmergency
                ? 'bg-red-500 text-white shadow-[0_4px_24px_rgba(224,82,82,0.4)] disabled:opacity-50'
                : 'bg-gradient-to-r from-[#C9A84C] to-[#E8C97A] text-[#0A0A0F] shadow-[0_4px_24px_rgba(201,168,76,0.35)] disabled:opacity-50',
          ...
          >
            {sending ? (
              ...
              <>
                ...
                {isEmergency ? 'Send Emergency Message' : 'Send Message'}
              </>
            )}
          </button>
          ...
          {isEmergency && (
            <p className="text-center text-[#5A5A72] text-xs mt-3 flex items-center justify-center gap-1">
              ...
              Emergency messages are permanent and cannot be deleted
            </p>
          )}
        </motion.div>
