  ...
  return (
    <AppShell showNav={false}>
      ...
      <div className="px-4 pb-8 max-w-lg mx-auto space-y-4">
        ...
        >
          ...
          <div className="px-5 pt-5 pb-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-1">Queue Status</p>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className={cn(
                    'text-xl font-bold',
                    queue?.is_open ? 'text-green-400' : 'text-[var(--text-muted)]'
                  )}>
            ...
            {queue?.is_open && (
              ...
              >
                ...
                <div className="h-2 bg-[var(--bg-card)] rounded-full overflow-hidden border border-[var(--border-subtle)]">
                  <motion.div
                    initial={{ width: 0 }}
                ...
                <p className="text-[var(--text-muted)] text-xs mt-1.5 text-right">{queueFillPercent}% full</p>
              </motion.div>
            )}
          ...
          <AnimatePresence>
            {showLimitPicker && (
              <motion.div
                ...
                className="border-t border-[var(--border-subtle)] px-5 py-4 bg-[var(--bg-secondary)]"
              >
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        customLimit === String(n)
                          ...
                          : 'bg-[var(--bg-card)] text-[var(--text-secondary)] border-[var(--border-subtle)] hover:border-[#C9A84C]/30'
                      )}
                    >
                      {n}
                    </button>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    min={1}
                    max={10000}
                    className="flex-1 bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-xl px-3 py-2 text-[var(--text-primary)] text-sm outline-none focus:border-[#C9A84C]/50"
                  />
                  <button
                ...
                <button onClick={() => setShowLimitPicker(false)} className="text-[var(--text-muted)] text-xs mt-2">
                  Cancel
                </button>
              </motion.div>
            )}
        ...
        >
          <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-4">Today</p>
        ...
        >
          <button
            ...
            className="w-full bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-2xl p-4 flex items-center gap-3 hover:border-[#C9A84C]/30 transition-all active:scale-[0.98]"
          >
            <div className="w-9 h-9 rounded-xl bg-purple-500/10 flex items-center justify-center flex-shrink-0">
              <BarChart2 className="w-4 h-4 text-purple-400" />
            </div>
            <span className="text-[var(--text-primary)] text-sm font-medium flex-1 text-left">View Detailed Analytics</span>
            <ChevronRight className={cn('w-4 h-4 text-[var(--text-muted)] transition-transform', showAnalytics && 'rotate-90')} />
          </button>
          ...
          <AnimatePresence>
            {showAnalytics && (
              ...
              >
                <div className="mt-2 bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-2xl p-5 space-y-3">
...
function StatCard({ icon, value, label, color }: {
  ...
  return (
    <div className={cn('rounded-2xl border p-3.5', colors[color])}>
      ...
      <p className="text-[var(--text-muted)] text-[10px] mt-0.5">{label}</p>
    </div>
  );
}
...
function TodayRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: number | string }) {
  return (
    <div className="flex items-center gap-2.5">
      <div className="w-7 h-7 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border-subtle)] flex items-center justify-center flex-shrink-0">
        {icon}
      </div>
      <div>
        <p className="text-[var(--text-primary)] text-sm font-semibold">{value}</p>
        <p className="text-[var(--text-muted)] text-[10px]">{label}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <div className="flex items-center gap-2 flex-1 min-w-0">
        {icon}
        <p className="text-[var(--text-secondary)] text-xs truncate">{label}</p>
      </div>
      <p className="text-[var(--text-primary)] text-sm font-semibold flex-shrink-0">{value}</p>
    </div>
  );
}
