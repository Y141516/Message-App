...
export default function HomePage() {
  ...
  const fetchQueues = async () => {
    try {
      ...
      // silently fail — show queue as closed
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className={cn(
                ...
                hasOpenQueue ? 'text-[#0A0A0F]' : 'text-[#5A5A72]'
              )}>
                Send Message
              </p>
              <p className={cn(
                'text-xs mt-0.5',
                hasOpenQueue ? 'text-black/60' : 'text-[#3A3A52]'
              )}>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2.5">
            ...
            <EmergencyCard
              ...
              label="Transport"
              color="blue"
              onClick={() => handleEmergency('transport')}
            />
            <EmergencyCard
              icon={<Siren className="w-5 h-5" />}
              label="Urgent"
              color="orange"
              onClick={() => handleEmergency('urgent')}
            />
          </div>
...
function EmergencyCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        'rounded-2xl p-3 border flex flex-col items-center gap-2 transition-all duration-150',
        colors[color]
      )}
    >
      {icon}
